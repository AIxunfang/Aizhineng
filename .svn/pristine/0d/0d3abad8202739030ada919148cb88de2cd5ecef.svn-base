     ...
     <el-col :span='24'>
             ...
             <div class="modeldataname">
                  ...
                  <span class="modelnameclasstop">创建时间:{{ timeFormattershowsecod(this.tarinmodelobj.trainingCreateTime)}}</span>
                   <span class="modelnameclasstop">训练开始时间:{{this.tarinmodelobj.trainingBeginTime==null ?'': timeFormattershowsecod(this.tarinmodelobj.trainingBeginTime)}}</span>
                    <span class="modelnameclasstop">训练完成时间:{{this.tarinmodelobj.trainingEndTime==null ? ''  : timeFormattershowsecod(this.tarinmodelobj.trainingEndTime)}}</span>
             </div>
      </el-col>
      <el-col :span="24">
      <el-col :span="14">
                                    ...
                                    <el-collapse-item>
                                        <template slot="title">
                                         ...
                                         <span class="lastborder" v-for="item in tfParams" :key="item.id">
                                                  <div class="databorder"><span class="variable"> 变量：</span ><span style="width:200px; display:inline-block">{{item.tfParamKey}}</span><span class="variable">值:</span> {{item.tfParamValue}}<span class="variable">目录类型:</span> <span v-if="item.tfParamType==0">文本</span><span v-if="item.tfParamType==1">数据</span><span v-if="item.tfParamType==2">模型</span>      </div>
                                         </span>
                                    </el-collapse-item>
                        </el-collapse>
                        </div>
